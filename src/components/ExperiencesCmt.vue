...
const experiences =
  instance.appContext.config.globalProperties.$JSONData.experiences;
const experienceTimeline = ref([
  ...experiences.slice(0, experiences.length),
  "",
]);
...
<template>
  ...
  <span class="column experience">
    <h1 id="titreExperience" class="formationTitres">
      {{ $t("experiences.experienceTitle") }}
    </h1>
    <h4 class="sousTitreFormations formationTitres">
      {{ $t("experiences.experienceSubtitle") }}
    </h4>
        ...
        <div class="card flex flex-wrap gap-12">
          ...
          >
            ...
            <template #content="experienceTimeline">
              <div
                class="conteneurBox experienceBox"
                v-if="experienceTimeline.item"
                @click="toggleDialog(experienceTimeline.index)"
              >
                <div class="information experienceInf">
                  <h5 class="annees">
                    <span
                      v-if="
                        DateUtils.isPastDate(experienceTimeline.item.dates[1])
                      "
                    >
                      {{
                        DateUtils.formatDateRange(
                          experienceTimeline.item.dates[0],
                          experienceTimeline.item.dates[1],
                          $i18n.locale,
                        )
                      }}
                    </span>
                    <span
                      v-else-if="
                        DateUtils.isFutureDate(experienceTimeline.item.dates[0])
                      "
                    >
                      {{
                        DateUtils.formatDateRange(
                      ...
                      <small id="avenir"
                        >({{ $t("experiences.experienceComing") }})</small
                      >
                    </span>
                    <span v-else>
                      {{
                  ...
                  <h4 class="titreExperience">
                    {{
                      experienceTimeline.item.contrat +
                      " " +
                      experienceTimeline.item.poste
                    }}
                  </h4>
                  <p class="expDesc">
                    {{ experienceTimeline.item.description }}
                  </p>
                  <span class="learnMore">
                    <i class="pi pi-info-circle"></i
                    ><a
                      ><p class="learnMoreText">{{ $t("words.learnmore") }}</p>
                    </a>
                  </span>
                </div>
                ...
                <div class="imgExperienceContainer">
                  <a>
                    <img
                      :src="experienceTimeline.item.image"
                ...
                <Dialog
                  ...
                  :modal="true"
                  :header="
                    $t('experiences.experienceModalTitle') +
                    ' ' +
                    experienceTimeline.item.poste +
                    ' (' +
                    experienceTimeline.item.entreprise +
                    ') '
                  "
                  class="custom-dialog"
...
#conteneurTimelineFormations
  > div
  > div
  > div
  > div.p-timeline-event-content
  > div.partieCachee.conteneurBox.experienceBox
  > span.expCacheContainerMissions {
  height: min-content;
  top: 0px;
}
