...
export function applyFilterStateClasses(filters, iconFilter, filterState) {
    const filterClass = filterState === 'activated' ? 'filterActivated' : 'filterDeactivated'
    const antiFilterClass = filterState !== 'activated' ? 'filterActivated' : 'filterDeactivated'
    
    filters.classList.add(filterClass)
    filters.classList.remove(antiFilterClass)
    iconFilter.classList.add(filterState === 'activated' ? 'pi-angle-double-down' : 'pi-angle-down')
    iconFilter.classList.remove(filterState === 'activated' ? 'pi-angle-down' : 'pi-angle-double-down')
}
