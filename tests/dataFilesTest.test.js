...
const dataEN = JSON.parse(
  fs.readFileSync(path.resolve(__dirname, "../src/data/data_en.json"), "utf-8"),
);
const dataFR = JSON.parse(
  fs.readFileSync(path.resolve(__dirname, "../src/data/data_fr.json"), "utf-8"),
);
...
describe("Test des fichiers JSON data_en.json et data_fr.json", () => {
  ...
  it("doit avoir le même nombre de clés principales", () => {
    expect(Object.keys(dataFR).length).toBe(Object.keys(dataEN).length);
  });

  it("doit avoir le même nombre d'éléments dans chaque section", () => {
    expect(Object.keys(dataFR.main_hard_skills).length).toBe(
      Object.keys(dataEN.main_hard_skills).length,
    );
  });
});
