...
describe('DateUtils', () => {
  ...
  describe('formatDateRange', () => {
    it('devrait formater correctement une plage de dates', () => {
      ...
      expect(result).toBe('01 juin 2023 - 10 juin 2023');
    });
    ...
    it('devrait retourner la date seule si les dates de début et de fin sont identiques', () => {
      ...
      expect(result).toBe('01 juin 2023');
    });

    it('devrait lancer une erreur si la date de début est null', () => {
      expect(() => DateUtils.formatDateRange(null, '2023-06-10')).toThrow('La date de début ou la date de fin est null');
    });
    ...
    it('devrait lancer une erreur si la date de début est supérieure à la date de fin', () => {
      expect(() => DateUtils.formatDateRange('2023-06-10', '2023-06-01')).toThrow('La date de début est supérieure à la date de fin');
    });

    it('devrait formater correctement une plage de dates dans une locale différente', () => {
      const result = DateUtils.formatDateRange('2023-06-01', '2023-06-10', 'en-US');
      expect(result).toBe('June 01, 2023 - June 10, 2023');
    });
  });
  ...
  describe('formatDate', () => {
    ...
    it('devrait formater une date dans une locale différente', () => {
      ...
      expect(result).toBe('June 01, 2023');
    });
  });
